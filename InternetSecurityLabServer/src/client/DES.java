...
public class DES {
    ...
    public static void main(String[] args) throws UnsupportedEncodingException{
        ...
        byte[] c=customDES.deal(origin.getBytes("ISO-8859-1"),1);
        System.out.println("密文：\n"+new String(c));
        for(int i = 0; i < c.length ; i++)
        	System.out.println(c[i]);
        byte[] p =customDES.deal(new String(c,"ISO-8859-1").getBytes("ISO-8859-1"),0);
//        byte[] p_d=new byte[100];
//                System.arraycopy(p,0,p_d,0,origin.getBytes().length);
        System.out.println("明文：\n"+new String(p));
        for(int i = 0; i < c.length ; i++)
        	System.out.println(new String(c,"ISO-8859-1").getBytes("ISO-8859-1")[i]);
    }
    /***代码运行结果：
     * 
     原文：
